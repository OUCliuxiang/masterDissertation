...
def loss_kl(teacher, student, T):
    return nn.KLDivLoss(reduction="batchmean")(F.log_softmax(student/T, dim=1),
...
def train(train_set, val_set, netname, dataset, model_t, model_s, gpus, _lr, _bs, epochs, T, alpha):
    ...
    #             momentum=MOMENTUM, weight_decay=WEIGHT_DECAY)
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model_t.parameters()), lr = 3e-4)
    ...
    steps = 0
    # train the Auxiliary Classifier
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_iter)) as t:
            for X, y in train_iter:
                ...
                loss = criterion(teacher_out, y)
                loss.backward()
                optimizer.step()
                
                t.set_description("T&AC Epoch [%i/%i] " % (epoch, epochs)) 

                train_loss += loss.cpu().item()
                train_acc += (teacher_out.argmax(dim=1) == y).sum().cpu().item()
            ...
            if val_acc/total_pic_val - max_acc > 1e-5:
                if len(gpus) > 1:
                    checkpoint = {'model': model_s.module,
                                ...
                                'epoch': epoch}
                    torch.save(checkpoint, '../weights/kd_ac_online_asy/{}_{}_best.pth'.format(netname, dataset, epoch))
                else:
    ...
    np.save("./data/kd_ac_online_asy/{}_{}_train_acc.npy".format(netname, dataset), train_acc_record)
    np.save("./data/kd_ac_online_asy/{}_{}_train_loss.npy".format(netname, dataset), train_loss_record)
    np.save("./data/kd_ac_online_asy/{}_{}_val_acc.npy".format(netname, dataset), val_acc_record)
    np.save("./data/kd_ac_online_asy/{}_{}_val_loss.npy".format(netname, dataset), val_loss_record)

    if len(gpus) > 1:
        checkpoint = {'model': model_s.module,
                        'model_state_dict': model_s.module.state_dict(),
                        'epoch': epoch}
        torch.save(checkpoint, '../weights/kd_ac_online_asy/{}_{}_last.pth'.format(netname, dataset, epoch))
    else:
        checkpoint = {'model': model_s,
                    'model_state_dict': model_s.state_dict(),
                    'epoch': epoch}
