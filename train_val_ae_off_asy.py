...
def train(train_set, val_set, netname, dataset, model_t, model_s, gpus, _lr, _bs, epochs, T, alpha):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_iter)) as t:
            for X, y in train_iter:
                ...
                t.set_description("AE Epoch [%i/%i] " % (epoch, epochs)) 
                steps += 1
            ...
            if val_acc/total_pic_val - max_acc > 1e-5:
                if len(gpus) > 1:
                    checkpoint = {'model': model_s.module,
                                ...
                                'epoch': epoch}
                    torch.save(checkpoint, '../weights/kd_ae_off_asy/{}_{}_best.pth'.format(netname, dataset, epoch))
                else:
    ...
    np.save("./data/kd_ae_off_asy/{}_{}_train_acc.npy".format(netname, dataset), train_acc_record)
    np.save("./data/kd_ae_off_asy/{}_{}_train_loss.npy".format(netname, dataset), train_loss_record)
    np.save("./data/kd_ae_off_asy/{}_{}_val_acc.npy".format(netname, dataset), val_acc_record)
    np.save("./data/kd_ae_off_asy/{}_{}_val_loss.npy".format(netname, dataset), val_loss_record)

    if len(gpus) > 1:
        checkpoint = {'model': model_s.module,
                        'model_state_dict': model_s.module.state_dict(),
                        'epoch': epoch}
        torch.save(checkpoint, '../weights/kd_ae_off_asy/{}_{}_last.pth'.format(netname, dataset, epoch))
    else:
        checkpoint = {'model': model_s,
                    'model_state_dict': model_s.state_dict(),
                    'epoch': epoch}
